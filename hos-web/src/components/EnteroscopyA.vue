  ...
  <div class="page-wrapper">
    <div class="page-header">
      <div class="header-left">
        <h1 class="page-title">肠镜检查管理</h1>
        <p class="page-desc">管理和查看肠镜检查记录</p>
      </div>
      <div class="header-actions">
        <el-button type="primary" icon="el-icon-refresh" @click="loadEnteroscopyList" circle size="small" title="刷新数据"></el-button>
      </div>
    </div>
    ...
    <el-card class="main-card">
      ...
      <div class="filter-section">
        <el-form :inline="true" :model="queryParams" class="filter-form" @submit.native.prevent>
          ...
          <el-form-item label="肠镜类型" class="filter-item">
            <el-input v-model="queryParams.enteroscopyType" placeholder="输入肠镜类型" clearable @keyup.enter.native="handleQuery" size="small"></el-input>
          </el-form-item>
          <el-form-item label="操作医生" class="filter-item">
      ...
      <el-table :data="enteroscopyTableData" stripe border v-loading="loading" class="data-table">
        <el-table-column prop="patientId" label="患者ID" width="120" align="center" fixed></el-table-column>
        <el-table-column prop="enteroscopyType" label="检查类型" min-width="120"></el-table-column>
        <el-table-column prop="examinationTime" label="检查时间" width="160" align="center">
          <template slot-scope="scope"><span class="time-cell">{{ formatDate(scope.row.examinationTime) }}</span></template>
        </el-table-column>
    ...
    <el-dialog
      ...
      width="600px"
      class="detail-dialog">
      <el-descriptions :column="2" border size="medium" v-if="currentEnteroscopy">
        <el-descriptions-item label="肠镜检查ID">{{ currentEnteroscopy.enteroscopyId }}</el-descriptions-item>
        <el-descriptions-item label="患者ID">{{ currentEnteroscopy.patientId }}</el-descriptions-item>
        <el-descriptions-item label="检查编号">{{ currentEnteroscopy.examinationNo || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检查类型">{{ currentEnteroscopy.enteroscopyType || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检查时间">{{ formatDate(currentEnteroscopy.examinationTime) }}</el-descriptions-item>
        <el-descriptions-item label="检查医生">{{ currentEnteroscopy.examineDoctor || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检查科室">{{ currentEnteroscopy.examineDept || '-' }}</el-descriptions-item>
        <el-descriptions-item label="上传时间">{{ formatDate(currentEnteroscopy.uploadTime) }}</el-descriptions-item>
        <el-descriptions-item label="报告结论" :span="2">{{ currentEnteroscopy.reportConclusion || '-' }}</el-descriptions-item>
        <el-descriptions-item label="报告文件" :span="2">
          <el-link v-if="currentEnteroscopy.reportUrl" :href="getPreviewUrl(currentEnteroscopy.reportUrl)" type="primary" target="_blank" icon="el-icon-view">查看报告</el-link>
    ...
    <el-dialog title="新增肠镜检查" :visible.sync="addDialogVisible" width="520px" :close-on-click-modal="false" class="form-dialog">
      <el-form :model="addForm" :rules="addRules" ref="addForm" label-width="100px" size="small">
        ...
        <el-form-item label="肠镜类型">
          <el-input v-model="addForm.enteroscopyType" placeholder="请输入肠镜类型"></el-input>
        </el-form-item>
    ...
    <el-dialog title="编辑肠镜检查" :visible.sync="editDialogVisible" width="520px" :close-on-click-modal="false" class="form-dialog">
    ...
    <el-dialog title="批量导入肠镜检查数据" :visible.sync="importDialogVisible" width="680px" :close-on-click-modal="false" class="form-dialog">
      <div class="import-tips">
        <el-alert title="Excel文件格式说明" type="info" :closable="false" show-icon>
          <template>
            <div class="tip-content">
              <p>请确保Excel文件包含以下列（表头行将被忽略）：</p>
              <p class="tip-columns">患者ID, 检查编号, 检查时间, 肠镜类型, 操作医生, 检查科室, 报告结论</p>
    ...
    <!-- XML导入对话框 -->
    <el-dialog title="XML导入肠镜检查数据" :visible.sync="xmlImportDialogVisible" width="500px" :close-on-click-modal="false">
      <el-form>
        ...
        <el-form-item>
          <el-link type="primary" :href="enteroscopyXmlTemplateUrl" download="肠镜检查导入模板.xml">下载XML模板</el-link>
        </el-form-item>
      </el-form>
      <span slot="footer">
...
export default {
  ...
  data() {
    return {
      ...
      queryTimeRange: [],
      enteroscopyTableData: [],
      loading: false,
      pageSize: 10,
      pageNum: 1,
      total: 0,
      detailDialogVisible: false,
      currentEnteroscopy: null,
      ...
      xmlFile: null,
      xmlFileList: [],
      enteroscopyXmlTemplateUrl: '/template/enteroscopy_import_template.xml'
    }
  },
  methods: {
    loadEnteroscopyList() {
      this.loading = true;
      this.$axios.post('/enteroscopyExamination/page', {
        ...
        .then(res => {
          if (res.code === 200) {
            this.enteroscopyTableData = res.data || [];
            this.total = res.total || 0;
          } else {
            this.$message.error('获取肠镜检查失败');
          }
        })
        .catch(error => {
          console.error('加载肠镜检查错误:', error);
          this.$message.error('加载肠镜检查出错');
        })
        .finally(() => {
          this.loading = false;
        });
    ...
    showLoadTemplateDialog() {
      this.$axios.get('/searchTemplate/list?templateType=enteroscopy')
        .then(res => res.data)
        .then(res => {
          if (res.code === 200) {
    ...
    handleExport() {
      ...
      this.$axios.post('/enteroscopyExamination/export', exportParams, {
        responseType: 'blob'
      })
        .then(res => {
          ...
          link.download = `肠镜检查数据_${new Date().toISOString().split('T')[0]}.xlsx`;
          link.click();
    ...
    handleViewDetail(row) {
      this.currentEnteroscopy = row;
      this.detailDialogVisible = true;
    },
    handleEdit(row) {
      this.editForm = {
        enteroscopyId: row.enteroscopyId,
        patientId: row.patientId,
        examinationNo: row.examinationNo || '',
        enteroscopyType: row.enteroscopyType || '',
    ...
    handleUpdate() {
      this.$refs.editForm.validate(valid => {
        if (valid) {
          ...
          this.$axios.post('/enteroscopyExamination/update', this.editForm)
            .then(res => res.data)
            .then(res => {
              if (res.code === 200) {
    ...
    handleDelete(row) {
      this.$confirm('确定要删除该肠镜检查记录吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$axios.delete('/enteroscopyExamination/' + row.enteroscopyId)
          .then(res => res.data)
          .then(res => {
            if (res.code === 200) {
              this.$message.success('删除成功');
    ...
    handleAdd() {
      this.$refs.addForm.validate(valid => {
        if (valid) {
          ...
          if (this.addForm.examinationNo) formData.append('examinationNo', this.addForm.examinationNo);
          if (this.addForm.enteroscopyType) formData.append('enteroscopyType', this.addForm.enteroscopyType);
          if (this.addForm.examinationTime) formData.append('examinationTime', this.formatDateForBackend(this.addForm.examinationTime));
          if (this.addForm.examineDoctor) formData.append('examineDoctor', this.addForm.examineDoctor);
          if (this.addForm.examineDept) formData.append('examineDept', this.addForm.examineDept);
          if (this.addForm.reportConclusion) formData.append('reportConclusion', this.addForm.reportConclusion);
          if (this.addForm.pdfFile) formData.append('pdfFile', this.addForm.pdfFile);

          this.$axios.post('/enteroscopyExamination/add', formData, {
    ...
    handleImport() {
      ...
      this.$axios.post('/import/enteroscopyExamination', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
        .then(res => res.data)
        .then(res => {
    ...
    handleDownloadErrorReport() {
      ...
      import('@/components/ExportErrorReport').then(module => {
        module.default.download(this.importResult.failedRows, '肠镜检查导入错误报告');
    ...
    submitXmlImport() {
      ...
      this.$axios.post('/import/enteroscopy/xml', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
        .then(res => {
          const data = res.data;
