  ...
  <div class="page-wrapper">
    <div class="page-header">
      <div class="header-left">
        <h1 class="page-title">核磁检查管理</h1>
        <p class="page-desc">管理和查看核磁检查记录</p>
      </div>
      <div class="header-actions">
        <el-button type="primary" icon="el-icon-refresh" @click="loadMriList" circle size="small" title="刷新数据"></el-button>
      </div>
    </div>
    ...
    <el-card class="main-card">
      ...
      <div class="filter-section">
        <el-form :inline="true" :model="queryParams" class="filter-form" @submit.native.prevent>
          ...
          <el-form-item label="检查时间" class="filter-item filter-item-wide">
      ...
      <el-table :data="mriTableData" stripe border v-loading="loading" class="data-table">
    ...
    <el-dialog
      ...
      width="600px"
      class="detail-dialog">
      <el-descriptions :column="2" border size="medium" v-if="currentMri">
        <el-descriptions-item label="核磁检查ID">{{ currentMri.mriId }}</el-descriptions-item>
        <el-descriptions-item label="患者ID">{{ currentMri.patientId }}</el-descriptions-item>
        <el-descriptions-item label="检查编号">{{ currentMri.examinationNo || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检查部位">{{ currentMri.examinationPart || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检查时间">{{ formatDate(currentMri.examinationTime) }}</el-descriptions-item>
        <el-descriptions-item label="检查医生">{{ currentMri.examineDoctor || '-' }}</el-descriptions-item>
        <el-descriptions-item label="检查科室">{{ currentMri.examineDept || '-' }}</el-descriptions-item>
        <el-descriptions-item label="上传时间">{{ formatDate(currentMri.uploadTime) }}</el-descriptions-item>
        <el-descriptions-item label="报告结论" :span="2">{{ currentMri.reportConclusion || '-' }}</el-descriptions-item>
        <el-descriptions-item label="报告文件" :span="2">
          <el-link v-if="currentMri.reportUrl" :href="getPreviewUrl(currentMri.reportUrl)" type="primary" target="_blank" icon="el-icon-view">查看报告</el-link>
    ...
    <el-dialog title="新增核磁检查" :visible.sync="addDialogVisible" width="520px" :close-on-click-modal="false" class="form-dialog">
    ...
    <el-dialog title="编辑核磁检查" :visible.sync="editDialogVisible" width="520px" :close-on-click-modal="false" class="form-dialog">
    ...
    <el-dialog title="批量导入核磁检查数据" :visible.sync="importDialogVisible" width="680px" :close-on-click-modal="false" class="form-dialog">
    ...
    <!-- XML导入对话框 -->
    <el-dialog title="XML导入MRI检查数据" :visible.sync="xmlImportDialogVisible" width="500px" :close-on-click-modal="false">
      <el-form>
        ...
        <el-form-item>
          <el-link type="primary" :href="mriXmlTemplateUrl" download="MRI检查导入模板.xml">下载XML模板</el-link>
        </el-form-item>
      </el-form>
      <span slot="footer">
...
export default {
  ...
  data() {
    return {
      ...
      xmlFile: null,
      xmlFileList: [],
      mriXmlTemplateUrl: '/template/mri_import_template.xml'
    }
  },
  methods: {
    loadMriList() {
      this.loading = true;
      this.$axios.post('/mriExamination/page', {
        ...
        .then(res => {
          if (res.code === 200) {
            this.mriTableData = res.data || [];
            this.total = res.total || 0;
          } else {
            this.$message.error('获取核磁检查失败');
          }
        })
        .catch(error => {
          console.error('加载核磁检查错误:', error);
          this.$message.error('加载核磁检查出错');
        })
        .finally(() => {
          this.loading = false;
        });
    ...
    showLoadTemplateDialog() {
      this.$axios.get('/searchTemplate/list?templateType=mri')
        .then(res => res.data)
        .then(res => {
          if (res.code === 200) {
    ...
    handleExport() {
      ...
      this.$axios.post('/mriExamination/export', exportParams, {
        responseType: 'blob'
      })
        .then(res => {
          ...
          link.download = `核磁检查数据_${new Date().toISOString().split('T')[0]}.xlsx`;
          link.click();
    ...
    handleUpdate() {
      this.$refs.editForm.validate(valid => {
        if (valid) {
          ...
          formData.append('mriId', this.editForm.mriId);
          ...
          this.$axios.post('/mriExamination/update', formData, {
    ...
    handleDelete(row) {
      this.$confirm('确定要删除该核磁检查记录吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        this.$axios.delete('/mriExamination/' + row.mriId)
          .then(res => res.data)
          .then(res => {
            if (res.code === 200) {
              this.$message.success('删除成功');
    ...
    handleAdd() {
      this.$refs.addForm.validate(valid => {
        if (valid) {
          ...
          this.$axios.post('/mriExamination/add', formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
          })
            .then(res => res.data)
            .then(res => {
    ...
    handleImport() {
      ...
      this.$axios.post('/import/mriExamination', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
        .then(res => res.data)
        .then(res => {
    ...
    handleDownloadErrorReport() {
      ...
      import('@/components/ExportErrorReport').then(module => {
        module.default.download(this.importResult.failedRows, '核磁检查导入错误报告');
    ...
    submitXmlImport() {
      ...
      this.$axios.post('/import/mri/xml', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
        .then(res => {
          const data = res.data;
