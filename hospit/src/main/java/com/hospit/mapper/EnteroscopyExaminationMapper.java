...
public interface EnteroscopyExaminationMapper extends BaseMapper<EnteroscopyExamination> {

    // 查询已存在的检查编号
    @Select("<script>" +
            "SELECT examination_no FROM enteroscopy_examination WHERE is_invalid = 0 AND examination_no IN " +
