...
import com.hospit.entity.MriImportTemplate;
...
public class MriExcelImportListener extends AnalysisEventListener<MriImportTemplate> {
    ...
    public MriExcelImportListener(ImportContext context) {
        this.context = context;
    }

    @Override
    public void invoke(MriImportTemplate template, AnalysisContext analysisContext) {
        if (isFirstRow) {
            isFirstRow = false;
            return;
        }
        ...
        try {
            MriExamination entity = convertToEntity(template);
    ...
    private String validateRow(MriImportTemplate template) {
    ...
    private String buildRawData(MriImportTemplate template) {
        ...
        if (template.getExaminationPart() != null) sb.append("检查部位:").append(template.getExaminationPart()).append(",");
    ...
    private MriExamination convertToEntity(MriImportTemplate template) {
        MriExamination entity = new MriExamination();
        entity.setPatientId(template.getPatientId().trim());
        entity.setExaminationNo(template.getExaminationNo());
        entity.setExaminationPart(template.getExaminationPart());
